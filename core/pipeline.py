...
from core.components.sam_segmenter import SAMSegmenter
from core.components.clip_classifier import CLIPClassifier
from settings import PipelineConfig, DEFAULT_CANDIDATE_CLASSES, ModelConfig

import os

# Default debug output: <project_root>/output/
...
class DinoSAMClipPipeline:
    ...
    def detect_and_classify(
        ...
        image_name: Optional[str] = None,   # 用于 debug 文件命名，防止视覆盖
    ) -> Dict:
        """
        ...
        # Debug 可视化：用 image_name 区分每张图的调试输出
        if len(valid_masks) > 0:
            import time as _t
            _tag = image_name or _t.strftime("%H%M%S_%f")[:10]
            _stem = os.path.splitext(_tag)[0]   # 去掉扩展名
            ...
            self.sam.visualize_prediction(
                ...
                save_path=f"{output_dir}/debug_{_stem}_sam.png"
            )
